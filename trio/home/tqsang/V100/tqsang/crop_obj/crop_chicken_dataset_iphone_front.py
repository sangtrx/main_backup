...
if __name__ == '__main__':
    ...
    for vid_name in tqdm(vid_names):
        ...
        frame_folder = '/home/tqsang/V100/tqsang/crop_obj/data_DrLe'
        ...
        while success:
            ...
            ret,thresh_up = cv2.threshold(imgray_up, 80,255,cv2.THRESH_BINARY) ## 44 + 30 calib
            thresh = np.concatenate((thresh_up, thresh_down), axis=0)
            # cv2.imwrite(os.path.join(frame_folder, '%d_imgray.jpg' % n_frames), thresh)  ## for testing
            ...
            if (x_crop + best_box[0] > x_crop) and (x_crop + best_box[2] < x_crop + w_crop):
                ...
                cv2.imwrite(os.path.join(frame_folder, 'img/%d_%d_%d.jpg' %(count_vid,n_frames,chick_count)), middle_frame) #### crop khung
                ...
                cv2.imwrite(os.path.join(frame_folder, 'mask/%d_%d_%d.jpg' %(count_vid,n_frames,chick_count)), mask) #### mask
