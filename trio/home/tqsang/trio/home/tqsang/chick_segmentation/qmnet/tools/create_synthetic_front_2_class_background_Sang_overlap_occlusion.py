...
def get_img_anns(imgId, catId, data_dir, data_type, coco):
    ...
    while anns[0]['bbox'][0] > 950 - anns[0]['bbox'][2]:
...
if __name__ == "__main__":
    ...
    background =  "/home/tqsang/V100/tqsang/crop_obj/background/bg1792.png"
    ANNOTATION_DIR = '/home/tqsang/V100/tqsang/crop_obj/front_2_class_new/mask'
    ...
    for catId in tqdm(catIds):  ## loop từng class
        ...
        for imgId in tqdm(imgIds):
            ...
            # a_plg_mask.save('/home/tqsang/chick_segmentation/my_dt2-main/tools/a/'+ a_iname +'.jpg')
            # continue
            ...
            if flag_a1 != 1:
                ...
                try:
                    ...
                    smooth_a1_plg = smooth_a1_plgs[max_i]
                ...
                ##### dán hình 
                paste_x1 = int(random.uniform(650,700)) # 960 - 960*2/3-10
                paste_y = 0 

                res_im.paste(a1_plg_img, 
                                (paste_x1, paste_y), 
                                mask=a1_plg_mask)
                ...
                # Gausian blur for edge
                a1_edge_mask = get_edges_mask(smooth_a1_plg, res_im.size[1], res_im.size[0])
                res_im = blur_edge_points(res_im, a1_edge_mask)
            ...
            '''if flag_a2 != 1:
                ...
                try:
                    ...
                    smooth_a2_plg = smooth_a2_plgs[max_i]
                ...
                ##### dán hình 
                paste_x2 = int(random.uniform(1920,2230)) # 960*2,960*3 - 960*2/3-10
                paste_y = 0 

                res_im.paste(a2_plg_img, 
                                (paste_x2, paste_y), 
                                mask=a2_plg_mask)
                ...
                # Gausian blur for edge
                a2_edge_mask = get_edges_mask(smooth_a2_plg, res_im.size[1], res_im.size[0])
                # res_im = blur_edge_points(res_im, a2_edge_mask)'''
            ...
            if flag_a1 != 1:
                ...
                ###### occlusion for 1st chick
                ##### filter for associated png annotations
                for root, _, files in os.walk(ANNOTATION_DIR):
                    annotation_files = filter_for_annotations(root, files, a_iname)
                    # go through each associated annotation
                    for annotation_filename in annotation_files:
                        poly_1 = np.asarray(Image.open(annotation_filename)
                            .convert('1')).astype(np.uint8)

                for root, _, files in os.walk(ANNOTATION_DIR):
                    annotation_files = filter_for_annotations(root, files, a1_iname)
                    # go through each associated annotation
                    for annotation_filename in annotation_files:
                        poly_2 = np.asarray(Image.open(annotation_filename)
                            .convert('1')).astype(np.uint8)
                
                mask_bg = np.zeros((1792,1792), np.uint8)

                ### add 1st mask first
                mask_bg[0:0+960, 0:0+950] += poly_1
                mask_bg[0:0+960, paste_x1:paste_x1+950] += poly_2*2

                mask_bg[mask_bg != 1] = 0
                
                # segmentation = binary_mask_to_polygon(mask_bg)
                # segmentation =  imantics.Mask(mask_bg).polygons()[0]
                # segmentation = segmentation.reshape((int(len(segmentation)/2), 2))

                try:
                    smooth_a_plgs = imantics.Mask(mask_bg).polygons()
                    max_len = 0
                    max_i = 0
                    smooth_a_plg_list = []
                    for i, plg in enumerate(smooth_a_plgs):
                        print('  ___'  + str(len(smooth_a_plg))+ '___ ')
                        if len(smooth_a_plg) < 3:
                            continue
                        smooth_a_plg = smooth_a_plgs[i]
                        smooth_a_plg = smooth_a_plg.reshape((int(len(smooth_a_plg)/2), 2))
                        smooth_a_plg = smooth_a_plg.reshape((smooth_a_plg.shape[0]*smooth_a_plg.shape[1])).tolist()
                        smooth_a_plg_list.append(smooth_a_plg) 
                        # if i == 1:
                        #     import pdb;pdb.set_trace()
                ...
                ann_dict["annotations"].append(
                    create_json_ann_format(
                            ii, smooth_a_plg_list, a_box, catId, seg_ii)
                )
                ...
                # cv2.imwrite('/home/tqsang/chick_segmentation/qmnet/tools/mask.jpg', a)
                # import pdb;pdb.set_trace()
            ...
            '''if flag_a2 != 1:
                ...
                # a2_box[0] = paste_x2
                # a2_box[1] = paste_y 
                # a2_box[2] = res_im.size[0] - a2_box[0] - 1
                # a2_box[3] = res_im.size[1] - a2_box[1] - 1
