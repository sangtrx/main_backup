...
class VideoDataSet(Dataset):
    ...
    def _get_dataset(self):
        ...
        if self.dataset_name == 'kitchen':
            self.video_ids = self.video_ids * 3
    ...
    def _load_item(self, index):
        ...
        if self.use_env is True:
            ...
            env_features = np.load(os.path.join(self.env_feature_dir, video_name + '.npy')).T
            env_features = torch.tensor(env_features).float()
        else:
            env_features = None
