...
class Solver:
    ...
    def train_epoch(self, data_loader, bm_mask, epoch, writer):
        ...
        for n_iter, (env_features, agent_features, agent_masks, obj_features, obj_masks, label_confidence, label_start, label_end) in enumerate(tqdm(data_loader)):
            ...
            # obj_features = obj_features.narrow(2,0,10)
            # obj_masks = obj_masks.narrow(2,0,10)
            ...
            ### fake input
            # f_agent_features = torch.rand(1, 100, 10, 1028).type(torch.float32).cuda()
            # f_agent_masks = torch.rand(1, 100, 10).type(torch.bool).cuda()
            # f_obj_features = torch.rand(1, 100, 10, 512).type(torch.float32).cuda()
            # f_obj_masks = torch.rand(1, 100, 10).type(torch.bool).cuda()

            # agent_features = f_agent_features
            # agent_masks = f_agent_masks
            # obj_features = f_obj_features
            # obj_masks = f_obj_masks
            ...
            #     if (count not in range(0,5+1)) and (count != 54 ) and (p.requires_grad):
            ...
            # pytorch_total_params = 0
            # for p in self.model.parameters():
            #     if (count in range(54,78+1)) and (count != 54 ) and (p.requires_grad):
            #         pytorch_total_params = pytorch_total_params + p.numel()
            #     count = count + 1
            # print(pytorch_total_params )
            # # break
    ...
    def inference(self, data_loader=None, split=None, batch_size=None):
        ...
        #     fake_obj_features = obj_features.narrow(2,0,10)
        #     fake_obj_masks = obj_masks.narrow(2,0,10)
        #     break
        ...
        with torch.no_grad():
            for video_names, env_features, agent_features, agent_masks, obj_features, obj_masks in tqdm(data_loader):
                ...
                #     global env_features,agent_features,agent_masks, obj_features, obj_masks
                #     # x1 = torch.FloatTensor(1, 100, 2048)
                #     # x2 = torch.FloatTensor(1, 100, 10, 1028)
                #     # x3 = torch.BoolTensor(1, 100, 10)
                #     # x4 = torch.FloatTensor(1, 100, 10, 512)
                #     # x5 = torch.BoolTensor(1, 100, 10)
                ...
                # summary(self.model, [(1, 100, 2048), (1, 100, 30, 1028),\
                #  (1, 100, 30), (1, 100, 20, 512), (1, 100, 20)])
                # break

                # from torchsummary import summary
                # summary(self.model, [( 100, 2048), ( 100, 30, 1028),\
                #  ( 100, 30), ( 100, 20, 512), ( 100, 20)])
                # break
                # from torchinfo import summary
                # summary(self.model, [( 100, 2048), ( 100, 30, 1028), ( 100, 30), ( 100, 20, 512), ( 100, 20)], dtypes=[torch.float32 ,torch.float32, torch.bool, torch.float32, torch.bool])
                ...
                # import time
                # AOE_tic = time.time()
                # BMN_toc = self.model(env_features, agent_features, agent_masks, obj_features, obj_masks)
