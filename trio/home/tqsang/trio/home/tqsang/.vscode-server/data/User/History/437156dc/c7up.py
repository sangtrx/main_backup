...
class RetrievalModelManager(models.BaseModelManager):
    ...
    # def forward(self, batch: Batch) -> RetrievalVisualEmbTuple:
        """
        ...
        with autocast(enabled=self.is_autocast_enabled()):
            ...
            # global vid_feat, vid_feat_mask, vid_feat_len
            # vid_feat, vid_feat_mask, vid_feat_len = batch.vid_feat, batch.vid_feat_mask, batch.vid_feat_len
            # #### đo FLOPS
            # def prepare_input(randowm=None):
            #     global vid_feat, vid_feat_mask, vid_feat_len
            #     x1 = vid_feat
            #     x2 = vid_feat_mask
            #     x3 = vid_feat_len                  
            #     return dict(features = x1, mask = x2, lengths = x3, hidden_state = None)

            # from ptflops import get_model_complexity_info
            
            # # import pdb;pdb.set_trace()

            # flops, params = get_model_complexity_info(net_vid_local, input_res=(1, 224, 224), 
            #                             input_constructor= prepare_input,
            #                             as_strings=True, print_per_layer_stat=False)

            # print('      - Flops:  ' + flops)
            # print('      - Params: ' + params)
            # print('      - len: ' , len(batch.vid_feat_len))
    ...
    #   - len:  1
            ...
            global vid_feat, vid_feat_mask, vid_feat_len
            vid_feat, vid_feat_mask, vid_feat_len = batch.clip_feat, batch.clip_feat_mask, batch.clip_feat_len
            ##### đo FLOPS
            def prepare_input(randowm=None):
                global vid_feat, vid_feat_mask, vid_feat_len
                x1 = vid_feat
            ...
            print('      - Flops:  ' + flops)
            print('      - Params: ' + params)
            print('      - len: ' , len(batch.clip_feat_len))
...
#       - len:  2
            ...
            # global vid_feat, vid_feat_mask, vid_feat_len , vid_context_hidden_
            # vid_feat, vid_feat_mask, vid_feat_len, vid_context_hidden_ = clip_emb_reshape, clip_emb_mask, batch.clip_num, vid_context_hidden
            # ##### đo FLOPS
            # def prepare_input(randowm=None):
            #     global vid_feat, vid_feat_mask, vid_feat_len,  vid_context_hidden_
            #     x1 = vid_feat
            #     x2 = vid_feat_mask
            #     x3 = vid_feat_len                  
            #     return dict(features = x1, mask = x2, lengths = x3, hidden_state = vid_context_hidden_)

            # from ptflops import get_model_complexity_info
            
            # # import pdb;pdb.set_trace()

            # flops, params = get_model_complexity_info(net_vid_global, input_res=(1, 224, 224), 
            #                             input_constructor= prepare_input,
            #                             as_strings=True, print_per_layer_stat=False)

            # print('      - Flops:  ' + flops)
            # print('      - Params: ' + params)
            # print('      - len: ' , len(batch.clip_num))
    ...
    #   - len:  1
            return RetrievalVisualEmbTuple(
                vid_emb, clip_emb, vid_context, clip_emb_reshape, clip_emb_mask, clip_emb_lens) , float(flops.split(' ')[0]), len(batch.clip_feat_len)
    ...
    def encode_text(self, batch: Batch) -> RetrievalTextEmbTuple:
        ...
        with autocast(enabled=self.is_autocast_enabled()):
            ...
            global vid_feat2, vid_feat_mask2, vid_feat_len2
            vid_feat2, vid_feat_mask2, vid_feat_len2 = batch.par_feat, batch.par_feat_mask, batch.par_feat_len
            #### đo FLOPS
            def prepare_input(randowm=None):
                global vid_feat2, vid_feat_mask2, vid_feat_len2
                x1 = vid_feat2
            ...
            flops, params = get_model_complexity_info(net_text_local, input_res=(1, 224, 224), 
                                        input_constructor= prepare_input,
                                        as_strings=True, print_per_layer_stat=False)

            print('      - Flops:  ' + flops)
            print('      - Params: ' + params)
            print('      - len: ' , len(batch.par_feat_len))

            paragraph_context = float(flops.split(' ')[0])
            l_paragraph_context = len(batch.par_feat_len)
            ...
            global vid_feat1, vid_feat_mask1, vid_feat_len1
            vid_feat1, vid_feat_mask1, vid_feat_len1 = batch.sent_feat, batch.sent_feat_mask, batch.sent_feat_len
            ##### đo FLOPS
            def prepare_input(randowm=None):
                global vid_feat1, vid_feat_mask1, vid_feat_len1
                x1 = vid_feat1
            ...
            print('      - Flops:  ' + flops)
            print('      - Params: ' + params)
            print('      - len: ' , len(batch.sent_feat_len))
            sentence_embedding = float(flops.split(' ')[0])
            l_sentence_embedding = len(batch.sent_feat_len)
            ...
            # global vid_feat, vid_feat_mask, vid_feat_len , vid_context_hidden_
            # vid_feat, vid_feat_mask, vid_feat_len, vid_context_hidden_ = sent_emb_reshape, sent_emb_mask, batch.sent_num, par_gru_hidden
            # ##### đo FLOPS
            # def prepare_input(randowm=None):
            #     global vid_feat, vid_feat_mask, vid_feat_len,  vid_context_hidden_
            #     x1 = vid_feat
            ...
            # flops, params = get_model_complexity_info(net_text_global, input_res=(1, 224, 224), 
            #                             input_constructor= prepare_input,
            #                             as_strings=True, print_per_layer_stat=False)

            # print('      - Flops:  ' + flops)
            # print('      - Params: ' + params)
            # print('      - len: ' , len(batch.sent_num))
    ...
    #   - len:  1
            ...
            return RetrievalTextEmbTuple(par_emb, sent_emb, par_context, sent_emb_reshape, sent_emb_mask, sent_emb_lens) , paragraph_context, l_paragraph_context, sentence_embedding, l_sentence_embedding
