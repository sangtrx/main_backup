...
if __name__ == "__main__":
    ...
    needed_folders = ['train2017', 'val2017', 'test2017', 'annotations']
    ...
    for catId in tqdm(catIds):  ## loop từng class
        ...
        for imgId in tqdm(imgIds):
            ...
            res_iname = splitext(a_iname)[0] + '_'
            

            ###### mask background
            res_mask = np.zeros(res_im.shape, np.uint8)
            ##### dán mask cho thằng đầu 
            ...
            res_mask = res_mask[paste_x:paste_x+len(binary_mask),paste_y:paste_y+len(binary_mask)] + binary_mask
            cv2.imwrite(os.path.join(frame_folder, 'mask/%s_%d_%d.jpg' %(vid_name,n_frames,chick_count)), mask)
            ...
            if flag_a1 != 1:
                ...
                paste_x1 = int(random.uniform(600,750)) # 832 max
                paste_y = 0 
            ...
            if flag_a1 != 1:
                ...
                poly_1  = smooth_a_plg
                poly1 = Polygon(poly_1)
                poly_2 = smooth_a1_plg
                poly2 = Polygon(poly_2)

                newdata.loc[0, 'geometry'] = poly1
                newdata.loc[1, 'geometry'] = poly2

                geoms = newdata['geometry'].tolist()
                ...
                try:
                    ...
                    geoms[0] = make_valid(geoms[0])
                    geoms[1] = make_valid(geoms[1])
                    overlaps = gpd.GeoDataFrame(gpd.GeoSeries([poly[0].intersection(poly[1]) for poly in itertools.combinations(geoms, 2) if poly[0].intersects(poly[1])]), columns=['geometry'])

                # erase the overlaps from the original geodataframe
                gdf2 = newdata.overlay(overlaps, how='difference')

                # import pdb;pdb.set_trace()
                if gdf2.loc[0,'geometry'].geom_type == 'Polygon':
                    smooth_a_plg_new = np.array(gdf2.loc[0,'geometry'].exterior.coords[:],dtype=np.int32)
                else:
                    # import pdb;pdb.set_trace()
                    
                    mycoordslist = [list(x.exterior.coords) for x in gdf2.loc[0,'geometry']]
                    smooth_a_plg_new = np.concatenate(mycoordslist, axis=0)
                    smooth_a_plg_new = smooth_a_plg_new.astype(np.int32)
                ...
                ann_dict["annotations"].append(
                    create_json_ann_format(
                            ii, smooth_a_plg_new, a_box, catId, seg_ii)
                )
