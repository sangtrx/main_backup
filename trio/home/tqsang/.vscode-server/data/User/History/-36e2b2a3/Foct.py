...
def detect_and_remove_blackish_pixels(image):
    ...
    new_mask[~mask] = False
    new_mask = Image.fromarray(new_mask.astype(np.uint8) * 255)

    return Image.fromarray(image_data), new_mask

def same_transformation(image1, image2):
    width, height = image1.size
    random_angle = np.random.uniform(-150, 150)
    random_offset = np.random.uniform(-0.2 * width, 0.2 * width, (4, 2))
    
    src_points = np.float32([[0, 0], [width, 0], [width, height], [0, height]])
    dst_points = src_points + random_offset
    
    matrix = cv2.getPerspectiveTransform(src_points, dst_points)
    result1 = cv2.warpPerspective(np.array(image1), matrix, (width, height), borderValue=(255, 255, 255))
    result2 = cv2.warpPerspective(np.array(image2), matrix, (width, height), borderValue=(255, 255, 255))

    result1 = Image.fromarray(result1)
    result2 = Image.fromarray(result2)
    result1 = result1.rotate(random_angle, resample=Image.BICUBIC)
    result2 = result2.rotate(random_angle, resample=Image.BICUBIC)
        ...
        for imgId in tqdm(imgIds):
            ...
            while count_new_images_per_sample < num_new_images_per_sample:
                ...
                while count_feathers_per_image < num_feathers_per_image:
                    ...
                    a_plg_img,a_plg_mask = detect_and_remove_blackish_pixels(a_plg_img)
                    
                    ### random transformation
                    a_plg_img,a_plg_mask = same_transformation(a_plg_img,a_plg_mask)
