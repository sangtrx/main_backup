...
def _evaluate_predictions_on_coco(
    ...
    if plot:
        ...
        C_M = ConfusionMatrix(nc=9, conf=0.65, iou_thres=0.1)
        stats = []
        for i in coco_gt.imgToAnns.keys():  # len val set
            ...
            try:
                ...
                continue
    C_M.print()
    plot_dir = "/home/tqsang/detectron2/"
    names = {k: v for k, v in enumerate(['wing','breast','tender','carcass','butt','thigh','drumstick','skin','fat'])}
    stats = [np.concatenate(x, 0) for x in zip(*stats)]
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats, plot=True, save_dir=plot_dir, names=names)
    C_M.plot(save_dir=plot_dir + 'confusion_matrix_rec.png', names=['wing','breast','tender','carcass','butt','thigh','drumstick','skin','fat'], rec_or_pred=0)
    C_M.plot(save_dir=plot_dir + 'confusion_matrix_pred.png', names=['wing','breast','tender','carcass','butt','thigh','drumstick','skin','fat'], rec_or_pred=1)
    # end
