    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line | and (box_y1 >= 5 and box_y2 <= y2-y-5): # Adjust area coordinates back to original frame
                ...
                cv2.imwrite('/home/tqsang/JSON2YOLO/data_test/img1.jpg',img)


                ################# IMG 1
                # img = cv2.imread('/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/datasets_classification_sync/test/defect/dot1_8_to_1.jpg')
                ...
                # Replace 'path/to/your/folder' with the actual path to the folder containing your images
                folder_path = '/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/datasets_classification_pad/test/defect/'

                # Use the glob module to get a list of all image files in the folder
                image_files = glob.glob(os.path.join(folder_path, '*.jpg'))  # Modify the file extension as needed

                # Loop through the image files and process them one by one
                for image_file in image_files:
                    ...
                    if predicted_class == 'normal': 
                        destination_folder = os.path.join('incorrect_imgs', 'defect')
                        os.makedirs(destination_folder, exist_ok=True)  # Create the destination folder if it doesn't exist
                        destination_path = os.path.join(destination_folder, os.path.basename(image_file))
                        shutil.copy(image_file, destination_path)(image_file, destination_path)
                # Replace 'path/to/your/folder' with the actual path to the folder containing your images
                folder_path = '/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/datasets_classification_pad/test/normal'

                # Use the glob module to get a list of all image files in the folder
                image_files = glob.glob(os.path.join(folder_path, '*.png'))  # Modify the file extension as needed
                ...
                # img = cv2.imread('/home/tqsang/JSON2YOLO/data_test/img1.jpg')
                img = cv2.imread('/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/datasets_classification_sync/test/defect/dot1_8_to_1.jpg')
                img = to_tensor(img)  # This does the same thing as `transforms.ToPILImage()` and `transforms.ToTensor()`
                img = normalize(img, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # Normalize the tensor
                img = img.unsqueeze(0)  # Add batch dimension
