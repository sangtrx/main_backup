...
if __name__ == '__main__':
    ...
    for vid_name in tqdm(vid_names):
        ...
        ## , '4_Trim_2.mp4' cho tập test
        # if vid_name not in ['4_Trim_1.mp4' , '5_Trim.mp4', '10_Trim.mp4']:  # train
        if vid_name not in ['4_Trim_2.mp4']:  #val
            continue
        
        frame_folder = '/home/tqsang/V100/tqsang/crop_obj/front_2_class_YOLO_val'
        # if vid_name in ['4_Trim_1.mp4' , '5_Trim.mp4', '10_Trim.mp4']:
        #     frame_folder = '/home/tqsang/V100/tqsang/crop_obj/front_2_class_YOLO/train'
        # else:
        #     frame_folder = '/home/tqsang/V100/tqsang/crop_obj/front_2_class_YOLO/val'

        try:
            # os.makedirs('/home/tqsang/V100/tqsang/crop_obj/front_2_class_YOLO')
        ...
        while success:
            ...
            red_imgray_up_half_lower = red_imgray[ red_height//4: red_height//2 , : ]
            ret,red_thresh_up_half_lower = cv2.threshold(red_imgray_up_half_lower, 20,255,cv2.THRESH_BINARY) #### 20 calib
            red_imgray_up_half_upper = red_imgray[: red_height//4 , : ]
            ret,red_thresh_up_half_upper = cv2.threshold(red_imgray_up_half_upper, 45,255,cv2.THRESH_BINARY) #### 20 calib

            ret,red_thresh_down = cv2.threshold(red_imgray_down, 20,255,cv2.THRESH_BINARY) #### 20 calib
            # ret,red_thresh_up = cv2.threshold(red_imgray_up, 45,240,cv2.THRESH_BINARY) ## 37 càng lớn càng mất 
            # red_thresh = np.concatenate((red_thresh_up, red_thresh_down), axis=0)

            red_thresh = np.concatenate((red_thresh_up_half_upper,red_thresh_up_half_lower, red_thresh_down), axis=0)

            # cv2.imwrite(os.path.join(frame_folder, '%d_imgray_threshr.jpg' % n_frames), red_thresh)
            ...
            thresh = red_thresh
            # cv2.imwrite(os.path.join(frame_folder, '%d_imgray_all.jpg' % n_frames), thresh)
            ...
            for c in contours:
                ...
                if 2000000>cv2.contourArea(c)>1500:
