...
def copy_images_with_wrong_predictions(model, phase, root_dir, dst_dir):
    model.eval()
    ...
    for inputs, labels, paths in dataloaders[phase]:
        ...
        wrong_indices = (preds != labels.data).nonzero(as_tuple=True)[0]
        if len(wrong_indices) > 0:
            wrong_predictions.extend([paths[i] for i in wrong_indices])

    # Copy the wrongly predicted images to the destination directory
    for path in wrong_predictions:
        # Obtain the original image path from the dataset root directory
        ...
        # Generate the destination image path
        dst_path = os.path.join(dst_dir, os.path.basename(path))
        
        # Copy the image
        shutil.copyfile(original_path, dst_path)

    print(f'Copied {len(wrong_predictions)} wrongly predicted images to {dst_dir}')
...
# Specify the dataset root directory and the destination directory for the wrongly predicted images
root_dir = '/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/datasets_classification/test'
dst_dir = 'wrong_predictions'

# Copy the wrongly predicted images
copy_images_with_wrong_predictions(model_ft, 'test', root_dir, dst_dir)
