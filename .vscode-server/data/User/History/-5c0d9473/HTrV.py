...
def rotate_bbox(bbox, angle, area_center):
    def rotate_point(point, center, angle):
        x, y = point
        cx, cy = center
        rx = math.cos(math.radians(angle)) * (x - cx) - math.sin(math.radians(angle)) * (y - cy) + cx
        ry = math.sin(math.radians(angle)) * (x - cx) + math.cos(math.radians(angle)) * (y - cy) + cy
        return rx, ry

    cx, cy = (bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2
    tl, tr, bl, br = (bbox[0], bbox[1]), (bbox[2], bbox[1]), (bbox[0], bbox[3]), (bbox[2], bbox[3])
    rotated_tl = rotate_point(tl, area_center, angle)
    rotated_tr = rotate_point(tr, area_center, angle)
    rotated_bl = rotate_point(bl, area_center, angle)
    rotated_br = rotate_point(br, area_center, angle)

    min_x = min(rotated_tl[0], rotated_tr[0], rotated_bl[0], rotated_br[0])
    max_x = max(rotated_tl[0], rotated_tr[0], rotated_bl[0], rotated_br[0])
    min_y = min(rotated_tl[1], rotated_tr[1], rotated_bl[1], rotated_br[1])
    max_y = max(rotated_tl[1], rotated_tr[1], rotated_bl[1], rotated_br[1])

    return [min_x, min_y, max_x, max_y]


def process_xml_file(xml_path, frame_number, areas, img_folder, json_data, img_id, ann_id, rotation_angle):
    ...
    for obj in root.findall("object"):
        ...
        area_center = ((crop_area[2] + crop_area[0]) / 2, (crop_area[3] + crop_area[1]) / 2)
        rotated = cropped.rotate(rotation_angle, resample=Image.BICUBIC, expand=False)
        rotated_bbox = rotate_bbox([xmin, ymin, xmax, ymax], rotation_angle, area_center)

        img_name = f"part{part}_{frame_number:08d}_{class_id}_{rotation_angle}.png"
        rotated.save(os.path.join(output_folder, img_name))
...
for part, (xml_folder, img_folder, xml_files, max_frame, train_split, areas) in enumerate([
    ...
    for i, xml_file in enumerate(xml_files):
        ...
        ann_id = process_xml_file(xml_path, frame_number, areas, img_folder, json_data, img_id, ann_id, rotation_angle)
        img_id += 1
