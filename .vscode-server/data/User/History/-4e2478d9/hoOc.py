...
def get_inter_area_ratio(bbox, area):
    inter_area = max(0, min(area[2], bbox[2]) - max(area[0], bbox[0])) * max(0, min(area[3], bbox[3]) - max(area[1], bbox[1]))
...
def find_best_area(areas, bbox):
    ...
    for area in areas:
        ratio = get_inter_area_ratio(bbox, area)
