    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                ## feather detection
                # Define the square area that contains the bounding box
                feather_area_x1 = max(0, int(box_x1 - (box_x2 - box_x1)))
                feather_area_y1 = max(0, int(box_y1 - (box_y2 - box_y1)))
                feather_area_x2 = min(cropped_frame.shape[1], int(box_x2 + (box_x2 - box_x1)))
                feather_area_y2 = min(cropped_frame.shape[0], int(box_y2 + (box_y2 - box_y1)))

                # Crop the square area from the frame
                feather_area = cropped_frame[feather_area_y1:feather_area_y2, feather_area_x1:feather_area_x2]

                # Perform inference on the feather area
                results_feather = model_feather.predict(feather_area, conf=0.8)
