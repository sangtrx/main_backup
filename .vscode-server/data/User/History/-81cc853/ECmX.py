        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                if square_x1 < square_x2 and square_y1 < square_y2:
                    ...
                    cropped_square = frame[square_y1:square_y2, square_x1:square_x2]

                    # Convert the cropped_square to BGR format
                    cropped_square_bgr = cv2.cvtColor(cropped_square, cv2.COLOR_RGB2BGR)
                    
                    # Save the cropped square as an image
                    cv2.imwrite('cropped_square.png', cropped_square_bgr)
                    ...
                    print("Invalid dimensions for square area")
                    cv2.imwrite(cropped_square, 'cropped_square.png')
