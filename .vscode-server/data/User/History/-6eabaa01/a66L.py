...
def parse_args():
    ...
    parser.add_argument('--chick', action='store_true',
                        help='For COCO models')
    ...
    def surgery(param_name, is_weight, tar_size, ckpt, ckpt2=None):
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(BASE_CLASSES):
                ...
                if 'cls_score' in param_name or 'mask_head' in param_name:
                    new_weight[IDMAP[c]] = pretrained_weight[idx]
                else:
                    new_weight[IDMAP[c]*4*factor:(IDMAP[c]+1)*4*factor] = \
                        pretrained_weight[idx*4*factor:(idx+1)*4*factor]
        else:
...
def combine_ckpts(args):
    ...
    """
    def surgery(param_name, is_weight, tar_size, ckpt, ckpt2=None):
        if not is_weight and param_name + '.bias' not in ckpt2['model']:
            return

        weight_name = param_name + ('.weight' if is_weight else '.bias')

        if '_sigma' in weight_name:
            param_name = param_name.split('_sigma')[0]

        weight_name_temp = param_name + ('.weight' if is_weight else '.bias')
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(NOVEL_CLASSES):
                if 'cls_score' in param_name or 'mask_head' in param_name:
                    ...
                    new_weight[IDMAP[c]*4*factor:(IDMAP[c]+1)*4*factor] = \
                        ckpt2_weight[i*4*factor:(i+1)*4*factor]
            if 'cls_score' in param_name:
...
def surgery_loop(args, surgery):
    ...
    tar_size_dict = {
        'roi_heads.mask_head.predictor': TAR_SIZE,
        'roi_heads.box_predictor.cls_score': TAR_SIZE + 1,
        'roi_heads.box_predictor.bbox_pred': TAR_SIZE * 4 * factor,
        'roi_heads.box_predictor.cls_score_sigma': TAR_SIZE + 1,
    }

    for idx, param_name in enumerate(args.param_name):
        surgery(param_name, True, tar_size_dict[param_name], ckpt, ckpt2)
        surgery(param_name, False, tar_size_dict[param_name], ckpt, ckpt2)
...
if __name__ == '__main__':
    ...
    factor = 2 if 'box_iou_uncertainty' in args.src1 else 1

    if 'mask' in args.src1:
        args.param_name += [
            'roi_heads.mask_head.predictor'
        ]

    if 'rcnn' in args.src1:
        args.param_name += [
            'roi_heads.box_predictor.cls_score',
            'roi_heads.box_predictor.bbox_pred',
        ]

    if 'bayesian' in args.src2:
        args.param_name += [
            'roi_heads.box_predictor.cls_score_sigma',
        ]
