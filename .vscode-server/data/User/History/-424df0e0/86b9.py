...
dataloaders = {"test": torch.utils.data.DataLoader(datasets.ImageFolder(os.path.join(data_dir, 'test'), data_transform), batch_size=64, shuffle=False, num_workers=4)}

class_names = image_datasets.classes
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

def plot_training(train_loss, val_loss, train_acc, val_acc, fold):
    ...
    plt.tight_layout()
    plt.savefig(f'training_plot_sync_fold_{fold}.png') # the plot is saved with a unique name for each fold
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=15, early_stop_epochs=4):
    since = time.time()
...
for fold, (train_ids, val_ids) in enumerate(kf.split(image_datasets)):
    ...
    dataloaders["val"] = torch.utils.data.DataLoader(
                      image_datasets,
                      batch_size=64, sampler=val_subsampler)
    ...
    plot_training(train_loss, val_loss, train_acc, val_acc, fold)
