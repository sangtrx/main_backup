...
def copy_files(src_img_folder, src_txt_folder, dst_folder, filenames, class_label):
    class_name = 'defect' if class_label == 1 else 'normal'
    ...
    for i, filename in enumerate(filenames):
        ...
        # Crop and save image for each bounding box in corresponding class directory
        with open(txt_src, 'r') as f:
            for line in f:
                ...
                with Image.open(img_src) as img:
                    ...
                    # Convert the bounding box coordinates from xyxy to pixel values
                    x_min = int(float(parts[2]) * img_width)
                    y_min = int(float(parts[3]) * img_height)
                    x_max = int(float(parts[4]) * img_width)
                    y_max = int(float(parts[5]) * img_height)

                    # Create destination directories if they don't exist
                    class_dst_dir = os.path.join(dst_folder, class_name)
                    os.makedirs(class_dst_dir, exist_ok=True)

                    # Crop image according to bounding box coordinates and save it
                    cropped_img = img.crop((x_min, y_min, x_max, y_max))
                    cropped_img.save(os.path.join(class_dst_dir, f"{filename_without_ext}_{i}.jpg"))
