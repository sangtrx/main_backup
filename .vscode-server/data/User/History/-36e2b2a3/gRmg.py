...
def random_transformation(image):
    ...
    dst_points = src_points + np.random.uniform(-0.2 * width, 0.2 * width, (4, 2))

    matrix = cv2.getPerspectiveTransform(src_points, dst_points)
    result = cv2.warpPerspective(np.array(image), matrix, (width, height), borderValue=(255, 255, 255))

    result = Image.fromarray(result)
    result = result.rotate(random_angle, resample=Image.BICUBIC)
