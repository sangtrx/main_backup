...
def has_blue_pixels(image, box, threshold=50):
    x, y, w, h = map(int, box)
    cropped_img = image[y:y + h, x:x + w]

    blue_channel = cropped_img[:, :, 0]
    green_channel = cropped_img[:, :, 1]
    red_channel = cropped_img[:, :, 2]

    blue_pixels = (blue_channel > threshold) & (green_channel < threshold) & (red_channel < threshold)
...
def process_cropped_frames(model, input_folder, output_folder, max_frame, start_frame):
    ...
    for frame_number in range(start_frame, max_frame + 10000):
        for side in ['L', 'R']:
            ...
            # Check if there are detections and no blue pixels in the detection box area
            valid_boxes = []
            for i, box in enumerate(results[0].boxes.xywh):
                if not has_blue_pixels(img_array, box):
                    valid_boxes.append(results[0].boxes[i])
