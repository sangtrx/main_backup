...
def copy_files(src_folder, dst_folder, filenames, class_label):
    for filename in filenames:
        img_src = os.path.join(src_folder, f"{filename}.jpg")
        txt_src = os.path.join(src_folder, f"{filename}.txt")
...
def process_dataset(src_folder, dst_base, list_file, class_label):
    with open(list_file, 'r') as f:
        filenames = [line.strip() for line in f]

    train_files, val_files, test_files = split_data(filenames)

    copy_files(src_folder, os.path.join(dst_base, 'train'), train_files, class_label)
    copy_files(src_folder, os.path.join(dst_base, 'val'), val_files, class_label)
    copy_files(src_folder, os.path.join(dst_base, 'test'), test_files, class_label)

# Define paths and class labels
src_folder = '/mnt/tqsang/dot1_data/original'
...
# Process datasets
process_dataset(src_folder, dst_base, defect_list_file, 1)
process_dataset(src_folder, dst_base, normal_list_file, 0)
