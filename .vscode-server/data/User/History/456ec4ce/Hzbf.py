    ...
    for filename in filenames:
        ...
        with open(txt_src, 'r') as f_src, open(txt_dst, 'w') as f_dst:
            for line in f_src:
                ...
                parts[2] = str(float(parts[2]) / img_width)
                parts[3] = str(float(parts[3]) / img_height)
                parts[4] = str(float(parts[4]) / img_width)
                parts[5] = str(float(parts[5]) / img_height)

                f_dst.write(' '.join(parts[0],parts[2],parts[3],parts[4],parts[5]))
