'estimate relative size of feathers'
import cv2
import glob
import os
import numpy as np
...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame):
    ...
    for i in range(len(feather_ratio_list)):
        if feather_ratio_list[i] < 0.05:
            feather_index = 0
        elif feather_ratio_list[i] < 0.1:
            feather_index = 1
        else:
            feather_index = 2
    ...
    for i in range(len(feather_on_skin_ratio_list)):
        if feather_on_skin_ratio_list[i] < 0.5:
            feather_on_skin_index = 0
        elif feather_on_skin_ratio_list[i] < 0.1:
    ...
    for i in range(len(feather_contours)):
        if feather_index_list[i] == 0:
            cv2.drawContours(output_frame, feather_contours, i, (0, 0, 255), 2)
        elif feather_index_list[i] == 1:
            cv2.drawContours(output_frame, feather_contours, i, (0, 0, 170), 2)
        else:
            cv2.drawContours(output_frame, feather_contours, i, (0, 0, 85), 2)
    for i in range(len(feather_on_skin_contours)):
        if feather_on_skin_index_list[i] == 0:
            cv2.drawContours(output_frame, feather_on_skin_contours, i, (0, 255, 0), 2)
        elif feather_on_skin_index_list[i] == 1:
            cv2.drawContours(output_frame, feather_on_skin_contours, i, (0, 170, 0), 2)
        else:
            cv2.drawContours(output_frame, feather_on_skin_contours, i, (0, 85, 0), 2)


    # Put text next to the image with the part counts with tier using the same color
