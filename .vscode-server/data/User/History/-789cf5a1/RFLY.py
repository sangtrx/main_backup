...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame):
    ...
    for i in range(len(feather_contours)):
        if feather_index_list[i] == 0:
            ...
            cv2.drawContours(output_frame, feather_contours, i, (0, 150, 170), 2)
        else:
            cv2.drawContours(output_frame, feather_contours, i, (150, 0, 85), 2)
    for i in range(len(feather_on_skin_contours)):
        if feather_on_skin_index_list[i] == 0:
            ...
            cv2.drawContours(output_frame, feather_on_skin_contours, i, (0, 170, 20), 2)
        else:
            cv2.drawContours(output_frame, feather_on_skin_contours, i, (0, 85, 45), 2)
