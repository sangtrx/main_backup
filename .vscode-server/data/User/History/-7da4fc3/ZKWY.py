...
def train_model(model, criterion, optimizer, scheduler, num_epochs=500, early_stop_epochs=50):
    since = time.time()
...
def plot_training(train_loss, val_loss, train_acc, val_acc):
    ...
    plt.subplot(1, 2, 1)
    plt.plot([loss.cpu().numpy() for loss in train_loss], label='Training Loss')
    plt.plot([loss.cpu().numpy() for loss in val_loss], label='Validation Loss')
    plt.legend()
    plt.title('Losses')
