...
## all classes
CHICK_CATEGORIES = [
    {"color": [220, 20, 60], "isthing": 1, "id": 1, "name": "normal"},
    {"color": [119, 11, 32], "isthing": 1, "id": 2, "name": "defect"},
    {"color": [0, 0, 142], "isthing": 1, "id": 3, "name": "feather"},
]

# Novel CHICK categories
CHICK_NOVEL_CATEGORIES = [
    {"color": [0, 0, 142], "isthing": 1, "id": 3, "name": "feather"}
]

def _get_chick_instances_meta():
    thing_ids = [k["id"] for k in CHICK_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in CHICK_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 3, len(thing_ids)
    # Mapping from the incontiguous COCO category id to an id in [0, 79]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in CHICK_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def _get_chick_fewshot_instances_meta():
    ret = _get_chick_instances_meta()
    novel_ids = [k["id"] for k in CHICK_NOVEL_CATEGORIES if k["isthing"] == 1]
    novel_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(novel_ids)}
    novel_classes = [
        k["name"] for k in CHICK_NOVEL_CATEGORIES if k["isthing"] == 1
    ]
    base_categories = [
        k
        for k in CHICK_CATEGORIES
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name == "chick":
        return _get_chick_instances_meta()
    elif dataset_name == "chick_fewshot":
        return _get_chick_fewshot_instances_meta()
