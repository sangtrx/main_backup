...
def perform_inference_and_draw_boxes(model, img, output_frame, color, part_name, ii, conf_threshold=0.3, show_score=False):
    ...
    # get the red channel of img, then get the biggest contour which is the carcass
    img_red = img[:,:,2]
    contours, hierarchy = cv2.findContours(img_red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    carcass = max(contours, key=cv2.contourArea)
    # draw the carcass
    cv2.drawContours(output_frame, [carcass], -1, (0, 0, 255), 2)
    ...
    for i, (box, score, cls) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
        ...
        if score >= conf_threshold:
            if carcass_contains_box(carcass, box):
...
def carcass_contains_box(carcass, box):
    box_x1, box_y1, box_x2, box_y2 = box.tolist()
    box_contour = np.array([[box_x1, box_y1], [box_x2, box_y1], [box_x2, box_y2], [box_x1, box_y2]])
    intersection = cv2.bitwise_and(carcass, carcass, mask=cv2.fillPoly(np.zeros_like(carcass), [box_contour.astype(np.int32)], 255))
