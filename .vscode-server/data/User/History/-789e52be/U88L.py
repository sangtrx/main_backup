...
def perform_inference_and_draw_boxes(model, img, output_frame, color, part_name, ii, conf_threshold=0.3, show_score=False):
    ...
    mask_carcass = np.zeros(img.shape, np.uint8)   
    cv2.drawContours(mask_carcass, carcass, -1, color=(255, 255, 255), thickness=cv2.FILLED)

    # draw the carcass for debugging
    cv2.drawContours(output_frame, mask_carcass, -1, color=(255, 255, 255), thickness=cv2.FILLED)
