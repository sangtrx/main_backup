...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        # Put text on the main area avg feather count, avg feather on skin count, avg wing count, avg skin count, avg flesh count, defect count, normal count
        cv2.putText(output_frame, f"Avg feather count: {round(featheravg_count, 2)}", (x, y-180), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[0], 2)
        cv2.putText(output_frame, f"Avg feather on skin count: {round(featheravg_count, 2)}", (x, y-150), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[1], 2)
        cv2.putText(output_frame, f"Avg wing count: {round(featheravg_count, 2)}", (x, y-120), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[2], 2)
        cv2.putText(output_frame, f"Avg skin count: {round(featheravg_count, 2)}", (x, y-90), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[3], 2)
        cv2.putText(output_frame, f"Avg flesh count: {round(featheravg_count, 2)}", (x, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[4], 2)
        ...
        #  draw a black box right next to the main area to display counts later
        cv2.rectangle(output_frame, (x2 + 10, y), (x2+300, y2), (0,0,0), -1)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                for i, part in enumerate(part_config):
                    ...
                    cv2.putText(output_frame, f'{part}: {part_count}', (x2 + 10, y + 40 * i + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
