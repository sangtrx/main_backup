...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        black_box = np.zeros((height, 520, 3), dtype=np.uint8)
        black_box.fill(0)
        black_box[y:y2, x2+20:x2+500] = (0, 0, 0)
        opacity = 0.8
        cv2.addWeighted(black_box, opacity, output_frame, 1 - opacity, 0, output_frame)
