...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"Avg feather on skin count: {round(featheravg_count, 2)}", (x, y-180), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[1], 2)
        cv2.putText(output_frame, f"Avg wing count: {round(featheravg_count, 2)}", (x, y-150), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[2], 2)
        cv2.putText(output_frame, f"Avg skin count: {round(featheravg_count, 2)}", (x, y-120), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[3], 2)
        cv2.putText(output_frame, f"Avg flesh count: {round(featheravg_count, 2)}", (x, y-90), cv2.FONT_HERSHEY_SIMPLEX, 1.5, colors[4], 2)
        ...
        cv2.rectangle(output_frame, (x2 + 10, y), (x2+350, y2), (0,0,0), -1)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                for i, part in enumerate(part_config):
                    ...
                    cv2.putText(output_frame, f'{part}: {part_count}', (x2 + 10, y + 45 * i + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.5, color, 2)
...
part_config = {
    ...
    'wing': {'model_path': '/home/tqsang/JSON2YOLO/wing_YOLO_det/runs/detect/yolov8x_wing_dot3_noMosaic/weights/best.pt', 'conf_threshold': 0.8},
    'skin': {'model_path': '/home/tqsang/JSON2YOLO/skin_YOLO_det/runs/detect/yolov8x_skin/weights/best.pt', 'conf_threshold': 0.3},
    'flesh': {'model_path': '/home/tqsang/JSON2YOLO/flesh_YOLO_det/runs/detect/yolov8x_flesh_dot3/weights/best.pt', 'conf_threshold': 0.3}
}
