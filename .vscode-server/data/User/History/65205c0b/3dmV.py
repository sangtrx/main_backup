...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"Avg feather count: {round(featheravg_count, 2)}", (x, y-180), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
        cv2.putText(output_frame, f"Avg feather on skin count: {round(featheravg_count, 2)}", (x, y-150), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
        cv2.putText(output_frame, f"Avg wing count: {round(featheravg_count, 2)}", (x, y-120), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
        cv2.putText(output_frame, f"Avg skin count: {round(featheravg_count, 2)}", (x, y-90), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
        cv2.putText(output_frame, f"Avg flesh count: {round(featheravg_count, 2)}", (x, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
