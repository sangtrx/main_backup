...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"feathers count: {featheravg_count}", (x, y-60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                ## feather detection
                # Crop another square frame that contains box_x1, box_y1, box_x2, box_y2
                square_size = int(max(box_x2 - box_x1, box_y2 - box_y1))
                center_x = (box_x1 + box_x2) / 2
                center_y = (box_y1 + box_y2) / 2
                square_x1 = int(center_x - square_size / 2)
                square_y1 = int(center_y - square_size / 2)
                square_x2 = square_x1 + square_size
                square_y2 = square_y1 + square_size
                
                # Crop the frame to the square area containing the box
                cropped_square_frame = frame[square_y1:square_y2, square_x1:square_x2]

                # Save the cropped square frame as an image
                output_filename = f'cropped_frame_{i}.png'  # You can adjust the filename as needed
                cv2.imwrite(output_filename, cropped_square_frame)
                import pdb;pdb.set_trace()
                
                # Perform inference on the square area
                results_feather = model_feather.predict(cropped_square_frame, conf=0.8)
                ...
                feather_count = sum(1 for i, (feather_box, feather_score, feather_class) in 
                                        ...
                                        feather_box[2]+square_x2 <= box_x2 and feather_box[3]+square_y2 <= box_y2)

                # # Count all the feathers within the box_x1, box_y1, box_x2, box_y2 area
                # feather_count = len(results_feather[0].boxes.xyxy)
                ...
                for i, (feather_box, feather_score, feather_class) in enumerate(zip(results_feather[0].boxes.xyxy, 
                    ...
                    if feather_box[0] >= box_x1 and feather_box[1] >= box_y1 and feather_box[2] <= box_x2 and feather_box[3] <= box_y2:
                        ...
                        cv2.rectangle(output_frame, 
                                    ...
                                    (int(feather_box_x2+x+square_x2), int(feather_box_y2+y+square_y2)), 
                                    (0, 255, 100), 2)  
...
model_path = '/home/tqsang/JSON2YOLO/feather_YOLO_det/runs/detect/yolov8x_brio_feather_det_512_NoRotate_NoGray/weights/best.pt'
...
areas = {
    '1-10': [1319, 598, 1319+1000, 598+1000],
}
...
output_dirs = ['/mnt/tqsang/test_vid_512']
...
for video_file in video_files:
    ...
    if 1 <= video_number <= 10:
        video_groups['1-10'].append(video_file)
