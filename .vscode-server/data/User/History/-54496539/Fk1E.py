...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"feathers count: {featheravg_count}", (x, y-50), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                ## feather detection
                # Perform inference on the area
                results = model_feather.predict(cropped_frame, conf=0.8)
                ...
                chicken_dict[chicken_id]["feather_count"] = feather_count
                ...
                if    x2-x-20<= box_x2 <= x2-x-5:
                    if hit_right == False and hit_left == True:
                        ...
                        chicken_dict[chicken_id]["average_feather_count"] = chicken_dict[chicken_id]["feather_count"] / len(chicken_dict[chicken_id]["classifications"])
