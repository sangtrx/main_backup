...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            # Correct the box coordinates for the original frame
            box_x1 += x
            box_x2 += x
            box_y1 += y
            box_y2 += y
            ...
            cv2.putText(output_frame, str(int(class_)), (int(box_x1), int(box_y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)

        # Draw the bounding rectangle for the main area
