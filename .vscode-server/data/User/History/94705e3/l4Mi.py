...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].xyxy, results[0].conf, results[0].cls)):
            ...
            cv2.rectangle(output_frame, (int(box_x1), int(box_y1)), (int(box_x2), int(box_y2)), (255, 0, 0), 2)
            cv2.putText(output_frame, str(class_), (int(box_x1), int(box_y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
...
output_path = '/mnt/tqsang/dot1_1_output_det.mp4'
