...
def main():
    ...
    part2_max_frame_number = 210 #4893
    ...
    for name, frames_folder, xml_folder, max_frame_number, areas in [
        ...
        for ann in annotations:
            ...
            ann_image_filename = [img["file_name"] for img in images if img["id"] == ann_image_id][0]
            ann_xml_filename = ann_image_filename.split('_')[1] + ".xml"
            if ann_xml_filename in train_xml_files:
                train_annotations.append(ann)
                matching_image = next(img for img in images if img["id"] == ann_image_id)
                if matching_image not in train_images:
                    train_images.append(matching_image)
            elif ann_xml_filename in val_xml_files:
                val_annotations.append(ann)
                matching_image = next(img for img in images if img["id"] == ann_image_id)
                if matching_image not in val_images:
                    val_images.append(matching_image)
    ...
    for name, _, _, max_frame_number, _ in [
        ("part1", part1_frames_folder, part1_xml_folder, part1_max_frame_number, part1_areas),
        ("part2", part2_frames_folder, part2_xml_folder, part2_max_frame_number, part2_areas),
    ]:
        for frame_number in range(int(max_frame_number * train_ratio)):
            for category_id in range(9):
                train_filenames.add(f"{name}_{frame_number:08d}_{category_id}.png")
                
        for frame_number in range(int(max_frame_number * train_ratio), max_frame_number):
            for category_id in range(9):
                val_filenames.add(f"{name}_{frame_number:08d}_{category_id}.png")

    train_images = [img for img in all_images if img["file_name"] in train_filenames]
    val_images = [img for img in all_images if img["file_name"] in val_filenames]
