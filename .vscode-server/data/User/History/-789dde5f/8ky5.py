...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame):
    ...
    cv2.putText(output_frame, f'{feather}: {count_feather}', text_position, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)       
    text_position = (10, 30 + 40 * 0)  # Update text position for each part
    cv2.putText(output_frame, f'{feather on skin}: {count_feather_on_skin}', text_position, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)    
