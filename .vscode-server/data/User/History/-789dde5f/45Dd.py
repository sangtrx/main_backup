...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame):
    ...
    # create a new results object to store the combined results in numpy
    combined_results = 
    ...
    combined_results[0].boxes.xyxy =  torch.tensor(xyxy, device='cuda:0') 
    combined_results[0].boxes.conf = torch.tensor(conf, device='cuda:0')
    combined_results[0].boxes.cls = torch.tensor(cls, device='cuda:0')
