...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame):
    ...
    # loop through the combined results if 2 boxes have IOU > 70, keep the one bigger area
    i = 0
    while i < len(combined_results.xyxy):
        box1 = combined_results.xyxy[i]
        score1 = combined_results.conf[i]
        for j in range(i+1, len(combined_results.xyxy)):
            box2 = combined_results.xyxy[j]
            score2 = combined_results.conf[j]
            if ultralytics.metrics.iou(box1, box2) > 0.7:
