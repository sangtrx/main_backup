...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"Last carcass number: {last_chick}", (x, y-400), cv2.FONT_HERSHEY_SIMPLEX, 1.7, (255,255,255), 2)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                cv2.putText(output_frame, f'feather: {feather_count}', (x2 + 20, y + 45 * 1 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, colors[0], 2)
                cv2.putText(output_frame, f'feather on skin: {feather_on_skin_count}', (x2 + 20, y + 45 * 2 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, colors[1], 2)
                ...
                for i, part in enumerate(['wing', 'skin', 'flesh']):
                    i += 3  # Start from 3 to skip 'feather' and 'feather on skin'
                    ...
                    # Put text in the black box with the part counts using the same color
                    cv2.putText(output_frame, f'{part}: {part_count}', (x2 + 20, y + 45 * i + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, color, 2)
                ...
                cv2.putText(output_frame, f'Current carcass: {chicken_id_counter} Frame: {frame_chick} Type: {predicted_class}', (x2 + 20, y + 45 * 1 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, colors[0], 2)
                ...
                if    x2-x-20<= box_x2 <= x2-x-5:
                    if hit_right == False and hit_left == True:
                        ...
                        last_chick = chicken_id + " " + avg_result
