    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                cv2.putText(output_frame, f'feather: {feather_count}', (x2 + 20, y + 45 * 0 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, colors[0], 2)
                cv2.putText(output_frame, f'feather on skin: {feather_on_skin_count}', (x2 + 20, y + 45 * 1 + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.7, colors[1], 2)
