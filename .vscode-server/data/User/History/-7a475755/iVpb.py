    ...
    while cap.isOpened():
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                if (box_x1 <= 5 and box_x2 > 5) and i not in chicken_dict:
                    chicken_dict[i] = {"crossed_left": True, "crossed_right": False, "classifications": [predicted_class]}

                # If chicken already crossed the left line, continue to update its classifications
                elif i in chicken_dict and chicken_dict[i]["crossed_left"] and not chicken_dict[i]["crossed_right"]:
                    chicken_dict[i]["classifications"].append(predicted_class)

                # If chicken crossed the right line
                if (box_x1 < x2-x-5 and box_x2 >= x2-x-5) and i in chicken_dict and chicken_dict[i]["crossed_left"]:
                    chicken_dict[i]["crossed_right"] = True

                    # Compute average classification result
                    avg_result = Counter(chicken_dict[i]["classifications"]).most_common(1)[0][0]
