...
def perform_inference_and_draw_boxes(model, img, output_frame, color, part_name):
    ...
    part_count = len(results[0].xyxy)
    
    # Draw bounding boxes for the detected objects
    for i, (box, score, cls) in enumerate(zip(results[0].xyxy, results[0].conf, results[0].cls)):
...
for image_path in glob.glob(os.path.join(image_directory, '*.png')):
    ...
    # Put text next to the image with the part counts
    text_position = (10, 30)
    for part, count in part_counts.items():
        text = f'{part}: {count}'
        cv2.putText(output_frame, text, text_position, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
