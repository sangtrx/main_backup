    ...
    for vid_name in tqdm(vid_names):
        ...
        while success:
            ...
            if (940 + best_box[0] > 940) and (940 + best_box[2] < 940 + 1400):
                ...
                ### defect 
                if (chick_count == 1) or (chick_count == 3) or (chick_count == 9): 
                    x_start = 940 + best_box[0] + 15 + randint(0,10)
                    y_start = best_box[3] - 15 + randint(0,10)
                    start_point = (x_start, y_start)
                    y_end = best_box[1] + 14 + randint(0,10)
                    end_point = (940 + best_box[2] -12 + randint(0,10) , y_end)
                    color = (0, 0, 255) ## đỏ
                    thickness = 6
                    image = cv2.rectangle(image, start_point, end_point, color, thickness)
                    score = random.choice(['99%','100%'])
                    image = cv2.putText(image, 'defect ' + score, (x_start, y_end - 10), cv2.FONT_HERSHEY_SIMPLEX, 
