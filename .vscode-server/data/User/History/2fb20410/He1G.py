...
class RetrievalTrainer(trainer_base.BaseTrainer):
    ...
    def validate_epoch(self, data_loader: data.DataLoader, val_clips: bool = False, save_embs: bool = False) -> (
        ...
        ### 
        avg_clip_embedding = 0
        avg_paragraph_context = 0
        avg_sentence_embedding = 0
        count = 0 
        total_clip_embedding = 0
        total_paragraph_context = 0
        total_sentence_embedding = 0
        sum_smart = 0
        for _step, batch in enumerate(data_loader):  # type: RetrievalDataBatchTuple
            ...
            with autocast(enabled=self.cfg.fp16_val):
                ...
                # import time
                # mart_tic = time.time()
                # visual_data, clip_embedding, l_clip_embedding = self.model_mgr.encode_visual(batch)
                # text_data, paragraph_context, l_paragraph_context, sentence_embedding, l_sentence_embedding = self.model_mgr.encode_text(batch)
                ...
                # total_clip_embedding += l_clip_embedding
                # total_paragraph_context += l_paragraph_context
                # total_sentence_embedding += l_sentence_embedding

                # avg_clip_embedding += clip_embedding
                # avg_paragraph_context += paragraph_context
                # avg_sentence_embedding += sentence_embedding
