...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        results = model.predict(cropped_frame, conf=0.95)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5) and (box_y1 >= 5 and box_y2 <= y2-y-5): # Adjust area coordinates back to original frame
                print('ok')
                ...
                cv2.imwrite(f'/mnt/tqsang/dot1_data/original/{video_path.split("/")[-1].split(".")[0]}_{frame_count}.jpg', cropped_frame)
                cv2.rectangle(cropped_frame, (int(box_x1), int(box_y1)), (int(box_x2), int(box_y2)), (255, 0, 0), 2)
                cv2.imwrite(f'/mnt/tqsang/dot1_data/visualize/{video_path.split("/")[-1].split(".")[0]}_{frame_count}.jpg', cropped_frame)
                with open(f'/mnt/tqsang/dot1_data/label/{video_path.split("/")[-1].split(".")[0]}_{frame_count}.txt', 'w') as f:
...
areas = {
    '1': [711, 391, 711+400, 391+400],
    '2-12': [510,389,510+500,389+500], # replace with actual area
    '13-29': [438,228,438+750,228+750], # replace with actual area
    '30-40': [589,451,589+550,451+550] # replace with actual area
}
