...
if __name__ == '__main__':
    ...
    for vid_name in tqdm(vid_names):
        ...
        if vid_name not in ['4_Trim_1.mp4' , '5_Trim.mp4', '10_Trim.mp4','4_Trim_2.mp4']: 
            continue
        vid_name = os.path.splitext(vid_name)[0]
        frame_folder = '/home/tqsang/V100/tqsang/crop_obj/front_2_class_1chick1img'
        ...
        while success:
            ...
            #     continue
            # if not (n_frames % 50 == 0): #2044:# 1920:
            ...
            if (x_crop + best_box[0] > x_crop) and (x_crop + best_box[2] < x_crop + w_crop):
                ...
                if current_chick == False:
                    ...
                    cv2.imwrite(os.path.join(frame_folder, 'img/%s_%d_%d.png' %(vid_name,n_frames,chick_count)), middle_frame) #### crop khung

                    mask = np.zeros(middle_frame.shape, np.uint8)   ### tạo shape giống image
                    cv2.drawContours(mask, new_contours, -1, color=(255, 255, 255), thickness=cv2.FILLED) 
                    # cv2.imwrite(os.path.join(frame_folder, 'mask/%d_%d.jpg' %(n_frames,chick_count)), mask) #### mask
                    cv2.imwrite(os.path.join(frame_folder, 'mask/%s_%d_%d.png' %(vid_name,n_frames,chick_count)), mask) #### mask
                    ...
                    cv2.imwrite(os.path.join(frame_folder, 'vis/%s_%d_%d.png' %(vid_name,n_frames,chick_count)), color_mask) #### mask
