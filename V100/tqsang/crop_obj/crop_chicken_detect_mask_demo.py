...
if __name__ == '__main__':
    ...
    for vid_name in tqdm(vid_names):
        ...
        out = cv2.VideoWriter(os.path.join(vid_dir_out,'demo_smartplant.avi'), fourcc, fps, (frame_width,frame_height))
        ...
        while success:
            ...
            if (940 + best_box[0] > 940) and (940 + best_box[2] < 940 + 1400):
                ...
                ### defect 
                if (chick_count == 1) or (chick_count == 3) or (chick_count == 9): 
                    x_start = 940 + best_box[0] + randint(0,2)
                    y_start = best_box[3] + randint(0,2)
                    start_point = (x_start, y_start)
                    y_end = best_box[1] + randint(0,2)
                    end_point = (940 + best_box[2] + randint(0,2) , y_end)
                    color = (0, 0, 255) ## đỏ
                    thickness = 7
                    image = cv2.rectangle(image, start_point, end_point, color, thickness)
                    score = random.choice(['99%','100%'])
                    image = cv2.putText(image, 'defect ' + score, (x_start, y_end - 5), cv2.FONT_HERSHEY_SIMPLEX, 
                    ...
                    mask = cv2.warpAffine(mask, M, (mask.shape[1], mask.shape[0]))
                    a =  mask*[0, 0, 255]
                    image = cv2.addWeighted(image, 1-0.0012, a, 0.0012, 0, dtype = cv2.CV_8U)
                ### normal                    
                else:
                    x_start = 940 + best_box[0]  + randint(0,2)
                    y_start = best_box[3]  + randint(0,2)
                    ...
                    image = cv2.rectangle(image, start_point, end_point, color, thickness)
                    score = random.choice(['99%','100%'])
                    image = cv2.putText(image, 'normal ' + score, (x_start, y_end - 10), cv2.FONT_HERSHEY_SIMPLEX, 
                            5, color, 8, cv2.LINE_AA)
