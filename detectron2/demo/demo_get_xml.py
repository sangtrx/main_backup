...
def create_annotation(predictions, frame_number, video_path, frame_width, frame_height):
    ...
    if num_instances == 0:
        # No objects detected in the frame, skip creating an XML file
        return
    ...
    for i in range(len(instances)):
        ...
        xmin.text, ymin.text, xmax.text, ymax.text = [str(x) for x in instances.pred_boxes[i].tensor.tolist()[0]]


    # Save the XML file
    tree = ET.ElementTree(annotation)
    # output_dir = "/mnt/tqsang/chicken_part2/xml"
    output_dir = "/mnt/tqsang/chicken_part1/xml"
    output_filename = os.path.join(output_dir, f"{(frame_number-1):08d}.xml")
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        out = cv2.VideoWriter(os.path.join('out_chicken_part1'+ '.avi'), fourcc, fps, (width,height))
