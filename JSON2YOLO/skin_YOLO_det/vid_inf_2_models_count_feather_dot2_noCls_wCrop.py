...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        cv2.putText(output_frame, f"Avg skin count: {round(featheravg_count, 2)}", (x, y-100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 2)
...
output_dirs = ['/mnt/tqsang/test_vid_skin']
...
for group, videos in video_groups.items():
    for video in videos:
        # if int(video.split('_')[2].split('.')[0]) in [3,4,7,8,9,40]: #test videos
            print(video)
            process_video(model, video, f'/mnt/tqsang/test_vid_skin/{video.split("/")[-1]}', areas[group])
